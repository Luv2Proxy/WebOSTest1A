...
export class UserOS {
  ...
  async boot(){this.write('VCPU-16 BIOS v0.2\n');this.write('Initializing hardware...\n');await this.system.boot();this.init=new InitProcess(this.system);await this.init.start();this.running=true;}
  ...
  async command(line){if(!this.running)return 'System is still booting.';return await this.init.command(line);}
